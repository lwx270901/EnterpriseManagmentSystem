...
if (count($task_result) == 0) {
    $result_control->create_result($_POST['task_id'], $_SESSION['user_id'], $_POST['filename'], $_POST['comment']);
} else {
    $prevResultId = intval($task_result[0]['ResultId']);
    $result_control->update_result($prevResultId, $_POST['filename'], $_POST['comment']);
}

// 2. Update the status of submitted task
$task_control->update_task_status($_POST['task_id'], $_POST['status']);
